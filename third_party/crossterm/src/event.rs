...
/// A command that enables mode 2031 color-scheme change notifications.
///
/// Terminals that support the mode (ghostty, kitty, contour, foot, …) report
/// every OS/terminal theme switch as `CSI ? 997 ; 1|2 n`, surfaced as
/// [`Event::ThemeModeChanged`]. Terminals that do not support the mode ignore
/// it. It should be paired with [`DisableThemeModeUpdates`] at the end of
/// execution.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl Command for EnableThemeModeUpdates {
    ...
    #[cfg(windows)]
    fn execute_winapi(&self) -> std::io::Result<()> {
        // The Windows console has no color-scheme report; VT-capable Windows
        // terminals consume the sequence via ANSI support.
        Ok(())
    }
}

/// A command that queries the terminal's current color scheme (`CSI ? 996 n`).
///
/// Terminals that support the query answer with the same `CSI ? 997 ; 1|2 n`
/// report surfaced as [`Event::ThemeModeChanged`]; terminals that do not
/// simply stay silent.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct RequestThemeMode;

impl Command for RequestThemeMode {
    fn write_ansi(&self, f: &mut impl fmt::Write) -> fmt::Result {
        f.write_str(csi!("?996n"))
    }
...
/// A command that disables mode 2031 color-scheme change notifications.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub enum Event {
    ...
    Resize(u16, u16),
    /// The terminal reported a color-scheme change (`CSI ? 997 ; 1|2 n`).
    /// Only sent by terminals after the application enables mode 2031
    /// (`CSI ? 2031 h`) color-scheme change notifications.
    ThemeModeChanged(ThemeMode),
}

/// Terminal color-scheme polarity carried by a mode 2031 report.
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, PartialOrd, PartialEq, Eq, Clone, Copy, Hash)]
pub enum ThemeMode {
    /// `CSI ? 997 ; 1 n` — the terminal switched to a dark color scheme.
    Dark,
    /// `CSI ? 997 ; 2 n` — the terminal switched to a light color scheme.
    Light,
}
