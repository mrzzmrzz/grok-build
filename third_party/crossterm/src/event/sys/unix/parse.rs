...
pub(crate) fn parse_csi(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let input_event = match buffer[2] {
        ...
        b'?' => match buffer[buffer.len() - 1] {
            ...
            // `n` is a final byte: the DSR-style report is complete now, so
            // never fall through to `None` (which would wait for more bytes
            // forever and wedge every later event in the buffer).
            b'n' => return parse_csi_theme_mode_report(buffer),
            _ => None,
        },
        b'0'..=b'9' => {
...
fn parse_csi_theme_mode_report(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    // ESC [ ? 997 ; 1|2 n — mode 2031 color-scheme change report.
    // Any other `CSI ? ... n` report is consumed via Err so an unrecognized
    // DSR clears the buffer instead of poisoning later events.
    assert!(buffer.starts_with(&[b'\x1B', b'[', b'?']));
    assert!(buffer.ends_with(&[b'n']));

    let params = std::str::from_utf8(&buffer[3..buffer.len() - 1])
        .map_err(|_| could_not_parse_event_error())?;
    ...
    Ok(Some(InternalEvent::Event(Event::ThemeModeChanged(mode))))
}
...
mod tests {
    ...
    #[test]
    fn test_theme_mode_report_dark() {
        assert_eq!(
            parse_event(b"\x1B[?997;1n", false).unwrap(),
            Some(InternalEvent::Event(Event::ThemeModeChanged(
                ThemeMode::Dark
            ))),
        );
    }
    ...
    #[test]
    fn test_theme_mode_report_light() {
        assert_eq!(
    ...
    #[test]
    fn test_theme_mode_report_incomplete_waits_for_more_bytes() {
        assert_eq!(parse_event(b"\x1B[?997;1", true).unwrap(), None);
    }

    #[test]
    fn test_unknown_private_dsr_report_clears_the_buffer() {
        // A final `n` must terminate the sequence one way or the other;
        // Ok(None) here would wedge every later event behind the report.
        assert!(parse_event(b"\x1B[?25;1n", false).is_err());
        assert!(parse_event(b"\x1B[?997;9n", false).is_err());
        assert!(parse_event(b"\x1B[?997n", false).is_err());
    }
