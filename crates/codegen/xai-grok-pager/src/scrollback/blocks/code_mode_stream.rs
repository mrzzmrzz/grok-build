...
mod tests {
    ...
    #[test]
    fn exec_source_renders_only_inferred_nested_tool() {
        let source =
            "const result = await tools.run_terminal_command({command: 'secret'}); text(result);";
    ...
    #[test]
    fn promise_all_renders_nested_tools_without_source() {
        let source = "await Promise.all([tools.read_file({file_path: '/secret'}), tools.grep_files({pattern: 'token'}), tools.mcp__server__run({id: 'private'})]);";
        let block = CodeModeStreamBlock::new(CodeModeStreamTool::Exec, source, 0);
