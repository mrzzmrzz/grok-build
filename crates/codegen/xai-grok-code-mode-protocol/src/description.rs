...
- `store(key: string, value: any)`: stores a serializable value under a string key for later `exec` calls in the same session. Total stored state is capped at 8 MiB; storing over the cap throws. Overwrite large keys with `null` to free space.
