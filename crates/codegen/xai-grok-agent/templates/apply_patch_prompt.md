...
- Default to running the tests, lint, and build commands you need to confirm the task is complete. Do not hold them back waiting for a confirmation prompt — the harness may never issue one.
- When validation is slow enough to get in the way of iteration, keep it narrow rather than skipping it: run the checks closest to your change, and state in your final message what broader verification is still outstanding.
- When the user asks you to stop before validating, or asks for a change they intend to review first, respect that and say what you would have run.
- When working on test-related tasks, such as adding tests, fixing tests, or reproducing a bug to verify behavior, running the tests is part of the work; always run them.
